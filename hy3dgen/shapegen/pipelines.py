...
class Hunyuan3DDiTPipeline:
    ...
    def enable_hicache(self, interval: int = 4, max_order: int = 1, first_enhance: int = 2,
                       ...
                       audit_budget: int = 0):
        """HiCache (Hermite) velocity-forecast acceleration on the flow-matching denoise
        loop — skip the DiT on (interval-1)/interval of the steps (arXiv:2508.16984).
        ``backend="dmd"`` swaps the polynomial forecaster for the exponential Prony/DMD
        one. Read natively by ``Hunyuan3DDiTFlowMatchingPipeline.__call__``."""
    ...
    def enable_dmd(self, interval: int = 4, first_enhance: int = 2, end_enhance: Optional[int] = None,
                   ...
                   max_horizon: Optional[int] = None, max_memory_mb: Optional[float] = None,
                   audit_budget: int = 0):
        """DMD/Prony exponential velocity-forecaster — the exponential analogue of HiCache.
        Reuses HiCache's compute/skip schedule but forecasts from raw velocity snapshots
        via fractional eigenvalue powers; Hermite covers warm-up. See ``hicache_dmd.py``."""
        cache_budget = _coerce_hicache_budget(
    ...
    def get_hicache_status(self):
        """Return the configured backend and whether a cache is enabled."""
    ...
    def get_hicache_telemetry(self):
        """Return the detached telemetry from the most recently completed run."""
...
class Hunyuan3DDiTFlowMatchingPipeline(Hunyuan3DDiTPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        with synchronize_timer('Diffusion Sampling'):
            # HiCache / DMD: forecast the CFG-combined velocity on skipped steps (skip the
            # whole DiT). No-op unless enable_hicache/enable_dmd was called.
            ...
            for i, t in enumerate(tqdm(timesteps, disable=not enable_pbar, desc="Diffusion Sampling:")):
                ...
                if budget_runtime is not None:
                    ...
                    budget_decision = budget_runtime.decide(
                        ...
                        # Hunyuan's fixed adapter selects Hermite for ``auto``;
                        # the selection is explicit and recorded in the manifest.
                ...
                if scheduled == "forecast" and not serve_forecast:
                    # hicache_decide has already incremented its skip counter. A
                    # budgeted audit or guard must turn that request into a real
                    # anchor so the next forecast starts from the new observation.
                    hicache["type"] = "full"
                    hicache["counter"] = 0
                ...
                if serve_forecast:
                    # forecast the final velocity from cached anchors; skip the DiT.
